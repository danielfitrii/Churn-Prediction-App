...
export default function ChurnPredictionApp() {
  ...
  const [modelInfo] = useState({
    ...
    xgboost: {
      description: `This gradient boosted tree model combines speed with high accuracy. It excels at capturing complex, non-linear interactions between features and tends to outperform other models on structured/tabular data.\n\nWhen to choose:\n- When you want the highest overall accuracy.\n- When feature interactions are complex and hard to model linearly.`,
      tip: 'Tip: For this model, Tenure, Contract Type, and Internet Service most affect predictions.'
    }
  });
  ...
  const [thresholdType, setThresholdType] = useState('f1');

  const showPredictionStrategy = settings.showPredictionStrategy;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Backend models expect exactly these 12 features (see backend/config.py FEATURE_NAMES)
      const encodedFeatures = [
        formData.seniorCitizen === "Yes" ? 1 : 0, // SeniorCitizen
        Number(formData.tenure), // tenure
        formData.phoneService === "Yes" ? 1 : 0, // PhoneService_Yes
        formData.multipleLines === "Yes" ? 1 : 0, // MultipleLines_Yes
        formData.internetService === "Fiber optic" ? 1 : 0, // InternetService_Fiber optic
        formData.internetService === "No" ? 1 : 0, // InternetService_No
        formData.onlineSecurity === "Yes" ? 1 : 0, // OnlineSecurity_Yes
        formData.streamingTV === "Yes" ? 1 : 0, // StreamingTV_Yes
        formData.streamingMovies === "Yes" ? 1 : 0, // StreamingMovies_Yes
        formData.contract === "One year" ? 1 : 0, // Contract_One year
        formData.contract === "Two year" ? 1 : 0, // Contract_Two year
        formData.paymentMethod === "Electronic check" ? 1 : 0, // PaymentMethod_Electronic check
      ];
      ...
      const predictionResult = {
        ...
        model: selectedModel === 'logistic' ? 'Logistic Regression' : selectedModel === 'randomForest' ? 'Random Forest' : 'XGBoost',
  ...
  return (
    <div className="w-full px-8 p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="w-full mb-6 p-6 bg-gray-50 rounded-lg customer-info-card">
        <h2 className="text-2xl font-bold text-gray-800 text-center mb-1">Customer Basic Information</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-gray-50 p-6 rounded-lg">
          ...
          <div className="bg-blue-50 p-4 rounded-lg mb-4">
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-semibold text-blue-800">
                    {selectedModel === "logistic" ? "Logistic Regression" : selectedModel === "randomForest" ? "Random Forest" : "XGBoost"} Model
                  </h3>
                </div>
              </div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className={`h-5 w-5 text-blue-600 transition-transform ${modelInfoOpen ? "rotate-180" : "rotate-0"}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            {modelInfoOpen && (
              <div id="model-info-content" className="mt-2 text-sm text-blue-700">
                <p className="whitespace-pre-line">{modelInfo[selectedModel].description}</p>
              </div>
            )}
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Tenure (months) <span className="text-gray-500 text-xs ml-1">(0-120)</span>
                  </label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Monthly Charges ($) <span className="text-gray-500 text-xs ml-1">(0-200)</span>
                  </label>
              ...
              <div className="mb-4 flex items-center whitespace-nowrap">
                <label className="block text-gray-700 font-medium flex items-center mr-4 mb-0 whitespace-nowrap">
                  <span className="relative group flex items-center">
                    <svg className="h-5 w-5 text-blue-400 cursor-pointer mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ minWidth: '20px' }}>
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                ...
                <div className="flex gap-4 items-center whitespace-nowrap">
                  <label className="inline-flex items-center whitespace-nowrap">
                    <input type="radio" className="form-radio" name="thresholdType" value="f1"
                      checked={thresholdType === 'f1'} onChange={() => setThresholdType('f1')} />
                    <span className="ml-2">Accurate (F1-optimized)</span>
                  </label>
                  <label className="inline-flex items-center whitespace-nowrap">
                    <input type="radio" className="form-radio" name="thresholdType" value="cost"
                      checked={thresholdType === 'cost'} onChange={() => setThresholdType('cost')} />
        ...
        <div className={`bg-gray-50 p-6 rounded-lg overflow-y-auto ${settings.showPredictionStrategy ? 'max-h-268' : 'max-h-258'}`}>
          ...
          {loading ? (
            ...
            prediction.riskLevel === "Error" ? (
              <div className="h-64 flex flex-col items-center justify-center">
                ...
                <div className="text-xl font-bold text-red-600 mb-2 text-center">Prediction Failed</div>
          ...
          {prediction && prediction.riskLevel !== "Error" && lastPredictedFormData && (
              ...
              <ul className="text-sm text-gray-600 space-y-2">
                {lastPredictedFormData.contract === "Month-to-month" && (
                  <li className="flex items-start">
                    <svg className="h-4 w-4 text-red-500 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                ...
                {(selectedModel === "randomForest" || selectedModel === "xgboost") && lastPredictedFormData.tenure < 6 && lastPredictedFormData.contract === "Month-to-month" && (
                  <li className="flex items-start">
                    <svg className="h-4 w-4 text-red-500 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
